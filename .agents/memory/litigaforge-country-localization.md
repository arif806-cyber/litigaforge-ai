...
- **GA is GA4 gtag.js, not a GTM container.** The id is `G-DHTR1SECG4` (GA4 measurement id) → must load `googletagmanager.com/gtag/js?id=G-…`. Do NOT switch to `gtm.js?id=G-…` (gtm.js expects a `GTM-…` container id and silently fails). Loader lives inline in `index.html`, fully deferred: gtag stub queues to dataLayer immediately, heavy script loads on `window load` → `setTimeout 3000` → `requestIdleCallback`. Latent pre-existing gap (not yet fixed): `src/lib/analytics.ts` `initGA`/`trackPageView` are dead (keyed on unset `VITE_GA4_ID`), so SPA route-change pageviews aren't tracked — only the initial config pageview fires.
