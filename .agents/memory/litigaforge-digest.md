...
- **Fail-closed SMTP**: `smtp_configured()` requires `SMTP_HOST`+`SMTP_USER`+`SMTP_PASSWORD`.
  ...
  — it never mock-delivers to real subscribers. As of 2026-06-16 `SMTP_HOST`/`SMTP_PORT`/
  `SMTP_USER` are set but **`SMTP_PASSWORD` is still unset** (user adds it manually), so
  the digest stays in single opt-in and can't deliver until the password is added + the
  backend restarted.
