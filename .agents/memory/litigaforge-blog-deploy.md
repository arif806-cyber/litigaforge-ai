...
NOTE: pipeline.yml lives in the **separate** `litigaforge-blog` repo (not this Replit project; `extract/`+`litigaforge-blog-work/` are stale local copies). The `GITHUB_TOKEN` env var present in this Replit env is **invalid (401 Bad credentials)** as of 2026-06-11 — can't push repo edits with it; would need a fresh `workflow`-scoped token.
