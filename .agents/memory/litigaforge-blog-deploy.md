...
**FIX APPLIED:** (1) cron moved `0 */2`→`17 */2` — **the `:17` tweak alone was NOT enough**; GitHub kept dropping nearly all scheduled runs (~1 of 24 fired over 2 days). (2) **Real fix = a Replit Scheduled Deployment** running `scripts/trigger-blog.sh` (in THIS monorepo) every 2h, which POSTs the pipeline's `workflow_dispatch` using the no-expiry PAT `GITHUB_PERSONAL_ACCESS_TOKEN_NOEXPIRE`. Replit's scheduler is reliable; GitHub's `schedule:` cron stays as a rare fallback (an occasional duplicate batch is harmless — pipeline has a `content-pipeline` concurrency group). **The main app deploy is autoscale/cloudrun (sleeps with no traffic) so it CANNOT host an in-server timer — that's why a separate Scheduled Deployment is required.** The Scheduled Deployment is user-created in the Publish UI (run cmd `bash scripts/trigger-blog.sh`, no build step needed; secrets are inherited from the repl).
