...
NOTE: pipeline.yml lives in the **separate** `litigaforge-blog` repo (not this Replit project; `extract/`+`litigaforge-blog-work/` are stale local copies). The old `GITHUB_TOKEN` env var was **invalid (401)**; a working PAT secret **`GITHUB_PERSONAL_ACCESS_TOKEN1`** (classic, full scopes incl. `repo`+`workflow`) is now available — editing `.github/workflows/*` REQUIRES the `workflow` scope. Read the value via the **bash tool** env, not code_execution (see replit-sandbox-secrets.md).
**FIX APPLIED:** cron moved `0 */2`→`17 */2` (off-peak minute) and committed to the blog repo `main`. Going forward scheduled runs should fire far more reliably; if it's still flaky, the stronger fix is an external pinger (Replit Scheduled Deployment / cron-job.org) hitting `workflow_dispatch`.
