...
## Topic clustering + Related Articles (SEO topical authority)
`pipeline.py` selects topics **cluster-first**: a `CLUSTERS` list of (country, legalArea, seed titles) is filled to `CLUSTER_MIN_POSTS` (default 4) per cluster — in-progress cluster first, then not-started, capped at the remaining slots — before falling back to Reddit trending. Cluster membership is counted from existing markdown **frontmatter on disk** (country + legalArea; `_area_matches` handles "A | B" multi-area).
**Key lesson — seed dedup is weak:** a CLUSTERS seed is skipped only when `_slugify(seed)` equals an existing **file slug**, but published slugs come from the AI's JSON and almost never match, so it will NOT catch a topic already covered under a different slug. Before adding/keeping a seed, check existing articles by *meaning*, not slug — a "wrongful termination" seed once would have published a 3rd near-duplicate.
**AI mislabels legalArea:** some existing posts are mistagged (a divorce guide tagged "Employment Law"), which skews cluster counts. The article prompt now passes an `area_hint` giving the model the exact legalArea string for cluster topics; still eyeball the first 2–3 runs' frontmatter.
Related Articles live in `src/pages/blog/[slug].astro`: frontmatter `getCollection('blog')` scores every other post (+2 same country, +1 shared legalArea), takes top 4, rendered with **relative** `/blog/<slug>` links so they work on both the Worker domain and the apex reverse-proxy.
