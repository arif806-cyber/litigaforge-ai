...
| Date | Change |
|---|---|
| Jun 18 | **Forge Workspace — Personalization Phase 2** — Personal Legal Twin deepens across 5 axes: (1) synthesis personalization now gated on `learning_enabled` toggle — analysis prompts only personalise when the user allows it; (2) argument-structure echo detection upgraded to **topological sort via graph edges** (Kahn's BFS over `edges_json`) — a dedicated **≋ ARGUMENT STRUCTURE ECHO** card appears when past matters share the same node-type sequence in the same structural order; (3) judgment cross-matter proactive alert — adding any IndianKanoon judgment fires a `cross_matter_hit` check and injects a proactive chip if the same case was cited in another of the user's matters; (4) **suggestion frequency downgrade** (not removal) for dismissed types — suppressed suggestions move to end of list at reduced frequency; ↩ Undo correctly restores full frequency via chronological event replay (`ORDER BY created_at DESC LIMIT N` + `reversed()` in Python ensuring `suggestion_reactivate` properly resets prior dismiss tallies); (5) **draft-style preference tracking** — 📋 Copy button on Drafting Agent emits `draft_accept` with detected `template_type` (LEGAL CONTENTION / PRAYER / etc.) and `word_count_bucket`; 👎 downvote on Drafting Agent emits `draft_reject`; twin summary surfaces preferred draft style in insight bullets |
...
| **Personal Legal Twin** | Opens in the **Twin** tab. Self-learning advocate profile built from your session history — which agents you trust, which nodes you add, which suggestions you accept. Personalises all 5 agents over time. Phase 2 additions: **argument-structure echo** detects when past matters share the same topological node order and surfaces a dedicated ≋ ECHO card; **cross-matter judgment alerts** fire a proactive chip when you add a judgment that appeared in another of your matters; **suggestion downgrading** demotes (not removes) dismissed suggestion types and restores full frequency on ↩ Undo; **draft-style tracking** learns your preferred draft template (LEGAL CONTENTION / PRAYER / etc.) from copy and thumbs actions; **learning_enabled toggle** lets you pause personalisation entirely from the Twin tab. |
